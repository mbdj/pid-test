...
// Mehdi 15/06/2022
//
// Version DEBUG avec contrôle par ATMega2560 qui permet la sortie série
...
//
// 2 versions d'asservissement : perso ou PID
// Perso : on corrige la consigne en cours par une correction proportionelle à l'erreur
// PID : commande = Kp * (erreur + Kd * dérivée(erreur) + Ki Intégrale(erreur))

// choix du microcontrôleur cible
// nb : penser à changer default_envs dans platformio.ini
#undef ATMEGA2560
#define ATTINY85

// choix du type de régulation
#define REGULATION_PERSO
#undef PID

// en mode DEBUG :
// sortie sur sortie série > nécessite de choisir ATMEGA2560 car pas assez de mémoire sur ATTINY85
// le potentiomètre n'est pas lu ; à la place un signal carré est généré comme consigne
#undef DEBUG
...
const int displayDelay{500}; // délai d'affichage. nb : on affiche peu souvent car l'affichage est long
...
#else
    // En mode debug la consigne est standardisée : elle est calculée : c'est un signal carré
